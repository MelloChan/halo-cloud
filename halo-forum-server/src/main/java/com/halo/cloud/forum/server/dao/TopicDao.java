...
public interface TopicDao {
    ...
    @Select("SELECT ht.id AS topicId,ht.title AS title,ht.user_id AS userId, " +
            ...
            "ORDER BY ht.gmt_create DESC")
    List<TopicDto> getTopicByPage();

    @Select("SELECT ht.id AS topicId,ht.title AS title,ht.user_id AS userId, " +
            "ht.type_id AS typeId,htt.type_name AS typeName,ht.gmt_create " +
            "FROM hl_topic ht LEFT JOIN hl_topic_type htt ON ht.type_id = htt.id " +
            "WHERE ht.type_id = #{typeId} AND `status` = 0 ORDER BY ht.gmt_create DESC")
    List<TopicDto> getTopicByTypeIdAndPage(@Param("typeId") int typeId);
    ...
    @Update("UPDATE hl_topic SET content = #{content},gmt_updated = NOW() WHERE id = #{id} AND user_id = #{userId} AND `status` = 0")
    int updateTopic(Topic topic);

    @Delete("UPDATE hl_topic SET `status` = #{status} WHERE id = #{id} AND user_id = #{userId}")
    int updateTopicStatus(@Param("status") int status, @Param("id") int id, @Param("userId") int userId);
